...
*/
	...
	if (finisec-startsec <= 0) {
		bbu_printf("Insufficient duration- Increase the LOOP count\n\r");
		return(1);
	}

	bbu_printf("Loops: %ld, Iterations: %d, Duration: %ld sec.\n\r",
			LOOP, II, (finisec-startsec)/TIMER_FREQUENCY);

	KIPS = (100.0*LOOP*II*TIMER_FREQUENCY)/(float)(finisec-startsec);
	if (KIPS >= 1000.0)
		bbu_printf("C Converted Double Precision Whetstones: %.1f MIPS\n\r", KIPS/1000.0);
	else
		bbu_printf("C Converted Double Precision Whetstones: %.1f KIPS\n\r", KIPS);
