...
#define QUIET_OUT
#define TIMER_FREQUENCY 13000000
#include <math.h>
#include <limits.h>
#include <float.h>
#include <stdio.h>
...
static void super_pi_help(char *argv[])
{
   bbu_printf (" %s - Command Super Pi benchmark\n\r", argv[0]);
   bbu_printf (" Usage: 1. %s <digits>\n\r", argv[0]);
   bbu_printf ("        2. %s dbg 0/1, turn on/off Super Pi log, 0 - off(default), 1 - on\n\r", argv[0]);
   bbu_printf ("        3. %s stress 0/1, start or stop Super Pi stress, 0 - stop(default), 1 - start\n\r", argv[0]);
   bbu_printf ("  <digits> - Number of digits of pi to calculate?\n\r");
}
#include "smp.h"

int DEF_MP_VAR(super_pi_stress) = {DEF_CORE_ARRAY(0)};
int super_pi_print = ON_OFF;
static void *DEF_MP_VAR(running_ao) = {DEF_CORE_ARRAY(0)};
#define SUPER_STRESS_FFT  200000
int stress_ao_stress;
int
super_pi_main (int argc, char *argv[], void *ao)
{
  ...
  DEF_GET_COREID
#ifndef QUIET_OUT
  bbu_printf("\n\rCalculation of PI using FFT and AGM, %s\n\r", PI_FFTC_VER);
#endif
  if(ao && ao != AO_STRESS)
    MP_VAR(running_ao) = ao;
  bbu_printf("CPU%d is running Super PI Test\n\r", smp_processor_id());
  nfft = SUPER_STRESS_FFT;
  ...
  switch(argc){
     case 2:
        ...
        }else if(strcmp((const char*)argv[1], "stress") == 0){
           if(ao == AO_STRESS)
               stress_ao_stress = 0;
           else
               MP_VAR(super_pi_stress) = 0;
           bbu_printf("Super PI stress Will Stop at once\n\r");
           return SCREEN_TEST_PASS;
        }else if(strcmp((char *)argv[1], "dbg") == 0){
            super_pi_print = 0;
            bbu_printf("Super PI log turn off\n\r");
            return SCREEN_TEST_PASS;
         }else
           nfft = atoi (argv[1]);
        break;
     case 3:
        if(strcmp((const char*)argv[1], "stress") == 0){
           int stress = conv_dec ((char *)argv[2]);
           if(ao == AO_STRESS)
              stress_ao_stress = stress;
           else
              MP_VAR(super_pi_stress) = stress;
           if(stress){
              nfft = SUPER_STRESS_FFT;
              super_pi_print = 0;
           }
            ...
            super_pi_print = atoi(argv[2]);
            if(super_pi_print)
               bbu_printf("Super PI log turn on\n\r");
            else
               bbu_printf("Super PI log turn off\n\r");
            return SCREEN_TEST_PASS;
         }
  ...
  nfft = 1 << log2_nfft;
  n = nfft + 2;
  a = (int *) bbu_malloc (2 * sizeof (int) + n * sizeof (DGTINT));
  b = (int *) bbu_malloc (2 * sizeof (int) + n * sizeof (DGTINT));
  c = (int *) bbu_malloc (2 * sizeof (int) + n * sizeof (DGTINT));
  e = (int *) bbu_malloc (2 * sizeof (int) + n * sizeof (DGTINT));
  i1 = (int *) bbu_malloc (2 * sizeof (int) + n * sizeof (DGTINT));
  i2 = (int *) bbu_malloc (2 * sizeof (int) + n * sizeof (DGTINT));
  d1 = (double *) bbu_malloc ((nfft + 2) * sizeof (double));
  d2 = (double *) bbu_malloc ((nfft + 2) * sizeof (double));
  d3 = (double *) bbu_malloc ((nfft + 2) * sizeof (double));
  if (d3 == NULL)
    {
      bbu_printf ("Allocation Failure!\n\r");
      //exit (1);
  ...
  DBG_PRINT(super_pi_print,"nfft= %d\n\rradix= %d\n\rerror_margin= %g\n\r", nfft, radix,
       err);
  DBG_PRINT(super_pi_print,"calculating %d digits of PI...\n\r", log10_radix * (n - 2));
  ...
  DBG_PRINT(super_pi_print,"AGM iteration\n\r");

  npow = 4;
  elap_time = ((double) timer_get_count(0) - (double) start_time)/TIMER_FREQUENCY;

  do
    {
      double start_loop_time = timer_get_count(0);
      npow *= 2;
      /* ---- e = (a + b) / 2 ---- */
      ...
      if (e[0] == 0)
    {
      ...
      loop_time =
    ((double) timer_get_count(0) - (double) start_loop_time)/TIMER_FREQUENCY;
      elap_time += loop_time;
      DBG_PRINT(super_pi_print,"precision= %d: %0.2f sec\n\r", 4 * nprc * log10_radix,
           loop_time);
    }
  while (4 * nprc <= n);
  start_time = timer_get_count(0);
  ...
  if(ao == AO_STRESS)
     bbu_sprintf (filename, "stress_pi%i.txt", log10_radix * (n - 2));
  else
     bbu_sprintf (filename, "core%dpi%i.txt", core_id, log10_radix * (n - 2));
#else
  bbu_sprintf (filename, "pi%i.txt", log10_radix * (n - 2));
#endif
  if(super_pi_print){
  f_out = mfopen (filename, "w");

  DBG_PRINT(super_pi_print,"writing %s...\n\r", filename);
...
//    fgets(filename,99,stdin);
//  }
  if(MP_VAR(super_pi_stress)){
      ComCmdEvt *cmdEvt = Q_NEW(ComCmdEvt, SUPER_PI_SIG);
      cmdEvt->argc = 10;
      QACTIVE_POST((QActive *)MP_VAR(running_ao), (QEvt *)cmdEvt, 0);
  }else if(stress_ao_stress){
      ComCmdEvt *cmdEvt = Q_NEW(ComCmdEvt, SUPER_PI_SIG);
      cmdEvt->argc = 10;
      QACTIVE_POST((QActive *)AO_STRESS, (QEvt *)cmdEvt, 0);
  }
  return (SCREEN_TEST_PASS);
}
...
#ifdef SUPER_PI_DEBUG
      bbu_printf ("n=%d, nfft=%d, prc=%d\n\r", n_nwt, nfft_nwt, prc);
#endif
