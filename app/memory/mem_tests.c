   ...
   switch(argc){
      ...
      case 3:{
         ...
         if(!strcmp((char *)argv[1], "dynamic") || 
            ...
            if( !is_test_buffer_safe((uint32_t)start_address, (uint32_t)byte_count)){
               return SCREEN_TEST_FIAL;
            }
         }
         if(INVALID_VIRTUAL_ADDRESS == get_physical(start_address)){
            ...
            if(is_mem_allocated(start_address, start_address+byte_count))
...
void temp_memory_copy_test(void)
{
   uint32_t i = 0;
   uint32_t src = 0x300000;
   uint32_t dest = 0x500000;
   uint32_t byte = 0x200000;
   uint32_t begin, end;

   for(i = 0; i < byte; i+=4)
   {
      BU_REG_WRITE(src+i, 0xdeadbeef);
   }
   
   begin = timer_get_count(0);
   bbu_mem_copy((uint32_t *)src, (uint32_t *)dest, byte);
   end = timer_get_count(0);
   end = end-begin;
   bbu_printf("Memory 0x200000 from 0x200000 to 0x500000(cacheable)\n\r");
   bbu_printf("Costed timer count: %d\n\r", end);
   ...
   bbu_printf("Memory 0x100000 from 0x7000000 to 0x7300000(non-cacheable)\n\r");
   bbu_printf("Costed timer count: %d\n\r", end);
