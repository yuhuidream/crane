...
{ 
	...
	MD5Init( &context); 
	MD5Update( &context, (uint8_t*)string, len ); 
	MD5Final(&context ); 
		
	for (i = 0; i < 16; i++) 
	{ 
		bbu_sprintf(&(output1[2*i]),"%02x",(uint8_t)context.digest[i]); 
		bbu_sprintf(&(output1[2*i+1]),"%02x",(uint8_t)(context.digest[i]<<4)); 
...
{ 
	MD5_CTX context; 	
	MD5Init( &context); 
	MD5Update( &context, (uint8_t*)array, len ); 
	MD5Final( &context ); 
