...
##add by Lianghu Xu
#Before moving per directory flags (CFLAGS, OPT_FLAGS,etc) to the end of global flags.
#Change OPT_FLAGS here so that we can assign OPT_FLAGS_$(d) := -O0 and rely on the gcc
#behavior to pick the last -O value
#for specific file we can do, e.g. $(OBJPATH)/qfsm_dis.o : OPT_FLAGS_$(d) := -O0
OPT_FLAGS = -O2 $(OPT_FLAGS_$(@RD))
...
#Add by Lianghu Xu
#How to work around the limitation of shell (Linux is 2M, Win is 32k)
#http://support.microsoft.com/kb/830473 
OBJLIBALL = $(DEP_OBJS) $(DEP_ARCH) $(DEP_LIBS) $(LIBS_$(@)) $(LDLIBS)
MAKECMD.$(SOEXT) = $(shell echo $(OBJLIBALL) > $(OBJPATH)/.objlibfile) $(LINK.cc) @$(OBJPATH)/.objlibfile -shared -o $@
DEFAULT_MAKECMD = $(shell echo $(OBJLIBALL) > $(OBJPATH)/.objlibfile) $(LINK.c) @$(OBJPATH)/.objlibfile -o $@ \
			&& $(OBJCOPY) -O binary $@ $(addsuffix .bin,$(basename $@))
