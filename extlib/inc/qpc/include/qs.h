...
#ifdef Q_SPY_EXT
//Multi-AO filter.
#define QS_AO_RECORD_STAT(ao_obj_filter, ao_pri)    QPSet64_hasElement(ao_obj_filter, ao_pri)
#define QS_AO_RECORD_IS_EMPTY(ao_obj_filter)        QPSet64_isEmpty(ao_obj_filter)

#define QS_AO_RECORD_ON(ao_pri)                     Q_SPIN_LOCK(&spy_filter_lock); \
                                                    QPSet64_insert(&QSAoSet, ao_pri); \
                                                    Q_SPIN_UNLOCK(&spy_filter_lock)
#define QS_AO_RECORD_OFF(ao_pri)                    Q_SPIN_LOCK(&spy_filter_lock); \
                                                    QPSet64_remove(&QSAoSet, ao_pri); \
                                                    Q_SPIN_UNLOCK(&spy_filter_lock)

//Multi-SM filter.
#define QS_SM_RECORD_STAT(sm_obj_filter, sm_index)  QPSet64_hasElement(sm_obj_filter, sm_index)
#define QS_SM_RECORD_IS_EMPTY(sm_obj_filter)        QPSet64_isEmpty(sm_obj_filter)

#define QS_SM_RECORD_ON(sm_index)                   Q_SPIN_LOCK(&spy_filter_lock); \
                                                    QPSet64_insert(&QSSmSet, sm_index); \
                                                    Q_SPIN_UNLOCK(&spy_filter_lock)
#define QS_SM_RECORD_OFF(sm_index)                  Q_SPIN_LOCK(&spy_filter_lock); \
                                                    QPSet64_remove(&QSSmSet, sm_index); \
                                                    Q_SPIN_UNLOCK(&spy_filter_lock)

#define QS_SM_INDEX(sm_obj)                         (sm_obj->super.index)
extern QSpinlock spy_filter_lock;
#endif
extern QSpinlock spy_buf_lock;
...
enum QSpyRecords {
    ...
    QS_QF_MALLOC,                       /**< a memory buffer was allocated  */
    QS_QF_MFREE,                            /**< a memory buffer was freed  */
    QS_QF_REALLOC,                  /**< a new memory buffer was allocated  */
    QS_QF_CALLOC,            /**< a memory buffer was allocated and zeroed  */
...
/** \brief output two uint8_t data elements without format information
* \note This function is only to be used through macros, never in the
* client code directly.
*/
...
#define QS_BEGIN(rec_, obj_) \
    QS_CRIT_STAT_ \
    QS_CRIT_ENTRY_(); \
    Q_SPIN_LOCK(&spy_filter_lock);\
    Q_SPIN_LOCK(&spy_buf_lock);\
    if (((QS_priv_.glbFilter[(uint8_t)(rec_) >> 3] \
          & (uint8_t)(1U << ((uint8_t)(rec_) & (uint8_t)7))) != (uint8_t)0) \
        && ((QS_priv_.apObjFilter == (void *)0) \
            || (QS_priv_.apObjFilter == (obj_)))) \
    { \
...
#ifdef Q_SPY_EXT
#define QS_BEGIN_AO_(rec_, objFilter_, pri) \
    QS_CRIT_ENTRY_(); \
    Q_SPIN_LOCK(&spy_filter_lock);\
    Q_SPIN_LOCK(&spy_buf_lock);\
    if (((QS_priv_.glbFilter[(uint8_t)(rec_) >> 3] \
          & (uint8_t)(1U << ((uint8_t)(rec_) & (uint8_t)7))) != (uint8_t)0) \
        && (QS_AO_RECORD_STAT(objFilter_, pri) \
            || QS_AO_RECORD_IS_EMPTY(objFilter_))) \
    { \
        QS_beginRec((uint8_t)(rec_));

#define QS_BEGIN_SM_(rec_, objFilter_, index) \
    QS_CRIT_ENTRY_(); \
    Q_SPIN_LOCK(&spy_filter_lock);\
    Q_SPIN_LOCK(&spy_buf_lock);\
    if (((QS_priv_.glbFilter[(uint8_t)(rec_) >> 3] \
          & (uint8_t)(1U << ((uint8_t)(rec_) & (uint8_t)7))) != (uint8_t)0) \
        && (QS_SM_RECORD_STAT(objFilter_, index) \
            || QS_SM_RECORD_IS_EMPTY(objFilter_))) \
    { \
        QS_beginRec((uint8_t)(rec_));
#endif
...
#ifdef Q_SPY_EXT
#define QS_BEGIN_NOCRIT_AO_(rec_, objFilter_, pri) \
    Q_SPIN_LOCK(&spy_filter_lock);\
    Q_SPIN_LOCK(&spy_buf_lock);\
    if (((QS_priv_.glbFilter[(uint8_t)(rec_) >> 3] \
          & (uint8_t)(1U << ((uint8_t)(rec_) & (uint8_t)7))) != (uint8_t)0) \
...
#define QS_BEGIN_NOCRIT_SM_(rec_, objFilter_, index) \
    Q_SPIN_LOCK(&spy_filter_lock);\
    Q_SPIN_LOCK(&spy_buf_lock);\
    if (((QS_priv_.glbFilter[(uint8_t)(rec_) >> 3] \
          & (uint8_t)(1U << ((uint8_t)(rec_) & (uint8_t)7))) != (uint8_t)0) \
...
/** \brief Output the assertion failure trace record
...
#define QF_QS_CRIT_ENTRY() \
    QS_BEGIN_NOCRIT_(QS_QF_CRIT_ENTRY, (void *)0, (void *)0) \
        QS_TIME_(); \
        QS_U8_((uint8_t)(++QS_priv_.critNest)); \
    QS_END_NOCRIT_()
...
/* QS private data .........................................................*/
typedef unsigned int QSCtr;     /**< QS ring buffer counter and offset type */

typedef struct QSPrivTag {
...
#ifdef Q_SPY_EXT
extern QPSet64 QSAoSet;
extern QPSet64 QSSmSet;
#endif
